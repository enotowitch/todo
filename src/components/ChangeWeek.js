...
export default function ChangeWeek(props) {
	...
	return (
		<div className="changeWeek">
			...
			{<img className="arrow week-arrow arrow_prev" src={arrow} style={arrowPrevStyle} onClick={() => props.changeWeek("prev")} />}
			<div>
				{weeks[lang][props.weekNum][0]}
				<span>&nbsp;&nbsp;-&nbsp;&nbsp;</span>
				{weeks[lang][props.weekNum][weeks[lang][props.weekNum].length - 1]}
			</div>
			{<img className="arrow week-arrow arrow_next" src={arrow} style={arrowNextStyle} onClick={() => props.changeWeek("next")} />}
		</div>
	)
}